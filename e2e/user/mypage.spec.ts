...
test.describe('회원 - 마이페이지', () => {
  ...
  test('신청 내역 탭 - 상태 필터를 사용한다', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-01-profile-tab.png', fullPage: true })

    // 신청 내역 탭 이동
    await page.getByRole('button', { name: '신청 내역' }).click()
    await expect(page.getByText('퇴근 게더링')).toBeVisible()
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-02-applications-all.png', fullPage: true })
    ...
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-03-applications-pending.png', fullPage: true })
    ...
    await page.waitForTimeout(300)
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-04-applications-confirmed.png', fullPage: true })
  })
  ...
  test('프로필 탭 - 프로필 정보를 수정한다', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-08-profile.png', fullPage: true })

    // 프로필 수정 버튼 (aria-label로 접근)
    await page.getByRole('button', { name: '프로필 수정' }).click()
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-09-edit-overlay.png', fullPage: true })
    ...
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-10-edit-filled.png', fullPage: true })
  ...
  test('후기 탭 - 작성한 후기 목록을 확인한다', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/user/mypage-12-reviews.png', fullPage: true })
  })
})
